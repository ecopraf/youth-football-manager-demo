export function setupLayout() {
  const demoUserStr = localStorage.getItem('yfm_demo_user');
  const demoUser = demoUserStr ? JSON.parse(demoUserStr) : null;
  
  const isDemoSession = localStorage.getItem('yfm_demo_session') === 'active';
  const userName = demoUser?.nome || 'Demo';
  
  // Badge demo per header
  const demoBadge = isDemoSession ? '<span id="demoBadge" style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:4px 10px;border-radius:12px;font-size:11px;font-weight:600;margin-left:8px;">🌱 Demo</span>' : '';
  ...
  style.textContent = `
    ...
    .sidebar-user-avatar { background: linear-gradient(135deg, #667eea, #764ba2) !important; }
  `;
