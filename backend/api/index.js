...
app.get('/api/health', async (req, res) => res.json({ status: 'ok', version: '2.8' }));
app.get('/api/workspaces', async (req, res) => { const { data } = await supabase.from('workspace').select('*'); res.json(data || []); });
app.get('/api/workspaces/:wsId/stagioni', async (req, res) => { const { data } = await supabase.from('stagione').select('*').eq('workspace_id', req.params.wsId); res.json(data || []); });
...
app.post('/api/stagioni/:stagioneId/squadre', async (req, res) => {
  ...
  const { data } = await supabase.from('squadra').insert({
    ...
    allenatore: b.allenatore, dirigente: b.dirigente, preparatore_atletico: b.preparatore_atletico, allenatore_portieri: b.allenatore_portieri
  }).select().single();
...
app.put('/api/squadre/:id', async (req, res) => {
  const b = req.body;
  await supabase.from('squadra').update({ nome: b.nome, categoria: b.categoria, allenatore: b.allenatore, dirigente: b.dirigente, preparatore_atletico: b.preparatore_atletico, allenatore_portieri: b.allenatore_portieri }).eq('id', req.params.id);
...
app.get('/api/squadre/:squadraId/calciatori', async (req, res) => {
  ...
  const { data } = await q;
  res.json((data||[]).map(r => ({ id: r.calciatore.id, nome: r.calciatore.nome, cognome: r.calciatore.cognome, dataNascita: r.calciatore.data_nascita, luogoNascita: r.calciatore.luogo_nascita, matricolaFigc: r.calciatore.matricola_figc, tipoDocumento: r.calciatore.tipo_documento, numeroDocumento: r.calciatore.numero_documento, rilasciatoDa: r.calciatore.rilasciato_da, numeroMaglia: r.numero_maglia, ruolo: r.ruolo, stato: r.stato })));
});

app.post('/api/squadre/:squadraId/calciatori', async (req, res) => {
  const c = req.body;
  const { data: cal } = await supabase.from('calciatore').insert({ workspace_id: '11111111-1111-1111-1111-111111111111', nome: c.nome, cognome: c.cognome, data_nascita: c.dataNascita, luogo_nascita: c.luogoNascita, matricola_figc: c.matricolaFigc, tipo_documento: c.tipoDocumento, numero_documento: c.numeroDocumento, rilasciato_da: c.rilasciatoDa }).select().single();
...
app.put('/api/calciatori/:id', async (req, res) => {
  const c = req.body;
  await supabase.from('calciatore').update({ nome: c.nome, cognome: c.cognome, data_nascita: c.dataNascita, luogo_nascita: c.luogoNascita, matricola_figc: c.matricolaFigc, tipo_documento: c.tipoDocumento, numero_documento: c.numeroDocumento, rilasciato_da: c.rilasciatoDa }).eq('id', req.params.id);
  if(c.numeroMaglia||c.ruolo) { const u = {}; if(c.numeroMaglia) u.numero_maglia = c.numeroMaglia; if(c.ruolo) u.ruolo = c.ruolo; await supabase.from('rosa').update(u).eq('calciatore_id', req.params.id); }
  res.json({ success: true });
});
...
app.put('/api/partite/:id', async (req, res) => { const p = req.body; await supabase.from('partita').update({ data_ora: p.dataOra, avversario: p.avversario, luogo: p.luogo, competizione: p.competizione, note: p.note }).eq('id', req.params.id); res.json({ success: true }); });
...
app.get('/api/squadre/:squadraId/statistiche-complete', async (req, res) => {
  try {
    ...
    let vittorie = 0, pareggi = 0, sconfitte = 0, gf = 0, gs = 0;
    const risultati = [];
    ...
    for (const p of (partite||[])) {
      ...
      // Simuliamo gol subiti basati su un pattern (approssimativo per demo)
      const golSubiti = Math.max(0, golFatti - 2 + Math.floor(Math.random() * 4));
    ...
    res.json({
      partiteGiocate: (partite||[]).filter(p => new Date(p.data_ora) < new Date()).length,
      partiteTotali: (partite||[]).length,
      vittorie, pareggi, sconfitte, golFatti: gf, golSubiti: gs, differenzaReti: gf - gs,
      risultati: risultati.sort((a,b) => new Date(b.dataOra) - new Date(a.dataOra))
    });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

// ── TOP PLAYERS CON MINUTAGGIO ──
app.get('/api/squadre/:squadraId/top-players', async (req, res) => {
  try {
    ...
    (eventi||[]).forEach(e => {
      if(!stats[e.calciatore_principale_id]) stats[e.calciatore_principale_id] = { gol:0, assist:0, presenze:0, minuti:0, ammonizioni:0 };
      stats[e.calciatore_principale_id].presenze++;
      if(e.tipo_evento_codice==='GOAL') { stats[e.calciatore_principale_id].gol++; stats[e.calciatore_principale_id].minuti += e.minuto; }
      ...
      if(e.tipo_evento_codice==='GOAL' && e.calciatore_secondario_id) {
        if(!stats[e.calciatore_secondario_id]) stats[e.calciatore_secondario_id] = { gol:0, assist:0, presenze:0, minuti:0, ammonizioni:0 };
    ...
    res.json({
      marcatori: result.sort((a,b) => b.gol-a.gol).slice(0,5),
      assistmen: result.sort((a,b) => b.assist-a.assist).slice(0,5),
      presenze: result.sort((a,b) => b.presenze-a.presenze).slice(0,5)
    });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

// Convocazioni, Distinta, Allenamenti (invariati)
...
app.post('/api/partite/:partitaId/convocazioni', async (req, res) => { const { calciatoreId, presente } = req.body; const { data: partita } = await supabase.from('partita').select('squadra_id').eq('id', req.params.partitaId).single(); await supabase.from('convocazione').upsert({ partita_id: req.params.partitaId, calciatore_id: calciatoreId, presente }); if(presente) { const { data: rosa } = await supabase.from('rosa').select('numero_maglia').eq('calciatore_id', calciatoreId).eq('squadra_id', partita.squadra_id).single(); await supabase.from('formazione_partita').upsert({ partita_id: req.params.partitaId, calciatore_id: calciatoreId, numero_maglia: rosa?.numero_maglia||99, posizione:'Panchina', capitano:false, vice_capitano:false }); } else { await supabase.from('formazione_partita').delete().eq('partita_id', req.params.partitaId).eq('calciatore_id', calciatoreId); } res.status(201).json({ success:true }); });
app.get('/api/partite/:partitaId/distinta', async (req, res) => { const { data: partita } = await supabase.from('partita').select('*, squadra:squadra_id(nome, stagione:stagione_id(nome, workspace:workspace_id(nome)))').eq('id', req.params.partitaId).single(); if(!partita) return res.status(404).json({ error:'Partita non trovata' }); const { data: formazione } = await supabase.from('formazione_partita').select('numero_maglia, posizione, capitano, vice_capitano, calciatore:calciatore_id(nome, cognome, data_nascita, matricola_figc, tipo_documento, numero_documento, rilasciato_da)').eq('partita_id', req.params.partitaId).order('numero_maglia'); res.json({ partita: { dataOra: partita.data_ora, avversario: partita.avversario, luogo: partita.luogo, competizione: partita.competizione }, societa: partita.squadra?.stagione?.workspace?.nome || 'ASD Albalonga', formazione: (formazione||[]).map(f => ({ numeroMaglia: f.numero_maglia, cognome: f.calciatore.cognome, nome: f.calciatore.nome, dataNascita: f.calciatore.data_nascita, capitano: f.capitano, viceCapitano: f.vice_capitano, matricolaFigc: f.calciatore.matricola_figc, tipoDocumento: f.calciatore.tipo_documento, numeroDocumento: f.calciatore.numero_documento, rilasciatoDa: f.calciatore.rilasciato_da, posizione: f.posizione })) }); });
...
app.get('/api/squadre/:squadraId/allenamenti/presenze', async (req, res) => { const { from, to } = req.query; let q = supabase.from('presenza_allenamento').select('id, data, presente, note, calciatore:calciatore_id(id, nome, cognome)').eq('squadra_id', req.params.squadraId).order('data', { ascending: false }); if(from) q = q.gte('data', from); if(to) q = q.lte('data', to); const { data: presenze } = await q; res.json((presenze||[]).map(p => ({ id: p.id, data: p.data, presente: p.presente, note: p.note, calciatoreId: p.calciatore.id, nome: p.calciatore.nome, cognome: p.calciatore.cognome }))); });
...
app.get('/api/squadre/:squadraId/allenamenti/summary', async (req, res) => { try { const { data: presenze } = await supabase.from('presenza_allenamento').select('calciatore_id, presente, data').eq('squadra_id', req.params.squadraId); const { data: rosa } = await supabase.from('rosa').select('calciatore:calciatore_id(id, nome, cognome)').eq('squadra_id', req.params.squadraId); const oggi = new Date(); const lunedi = new Date(oggi); lunedi.setDate(oggi.getDate()-((oggi.getDay()+6)%7)); const lunediStr = lunedi.toISOString().split('T')[0]; const summary = {}; (rosa||[]).forEach(r => { summary[r.calciatore.id] = { nome:r.calciatore.nome, cognome:r.calciatore.cognome, totali:0, presenti:0, assenti:0, settimanali:0, presentiSett:0, assentiSett:0 }; }); (presenze||[]).forEach(p => { if(summary[p.calciatore_id]) { summary[p.calciatore_id].totali++; if(p.presente) summary[p.calciatore_id].presenti++; else summary[p.calciatore_id].assenti++; if(p.data >= lunediStr) { summary[p.calciatore_id].settimanali++; if(p.presente) summary[p.calciatore_id].presentiSett++; else summary[p.calciatore_id].assentiSett++; } } }); res.json(summary); } catch(err) { res.status(500).json({ error:err.message }); } });
app.get('/api/squadre/:squadraId/statistiche', async (req, res) => { const { data: partite } = await supabase.from('partita').select('id').eq('squadra_id', req.params.squadraId); const { count } = await supabase.from('rosa').select('*', { count:'exact', head:true }).eq('squadra_id', req.params.squadraId); res.json({ partiteGiocate: (partite||[]).length, calciatoriInRosa: count||0 }); });
