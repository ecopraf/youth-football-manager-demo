...
app.post('/api/squadre/:squadraId/calciatori-batch-insert', async (req, res) => { const { giocatori } = req.body; const calciatori = giocatori.map(g => ({ workspace_id: '22222222-2222-2222-2222-222222222222', nome: g.nome, cognome: g.cognome, ruolo: g.ruolo, data_nascita: g.data_nascita })); const { data: calData } = await supabase.from('calciatore').insert(calciatori).select(); if (calData) { const rosa = calData.map((c, i) => ({ squadra_id: req.params.squadraId, calciatore_id: c.id, numero_maglia: giocatori[i].numero, ruolo: giocatori[i].ruolo, stato: 'Attivo' })); await supabase.from('rosa').insert(rosa); } res.status(201).json({ created: calData?.length || 0 }); });
